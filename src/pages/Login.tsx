...
export default function Login() {
  ...
  async function signInWithGoogle(forceAccountPicker = false) {
    ...
    if (forceAccountPicker) {
      ...
      queryParams['login_hint'] = lastEmail
    }
    try {
        ...
        <div className="w-full max-w-md">
          ...
          <div className="flex flex-col gap-4">
            ...
            <button onClick={() => signInWithGoogle(false)} disabled={Boolean(authLoading)} className="w-full flex items-center justify-center gap-3 bg-surface-container-lowest border border-outline-variant rounded-xl px-6 py-4 font-bold text-on-background hover:bg-surface-container-high transition-colors shadow-sm text-sm group disabled:opacity-60">
              ...
              <span>{authLoading === 'google' ? 'Redirecting…' : 'Continue with Google'}</span>
            ...
            <button onClick={signInWithAnotherGoogleAccount} disabled={Boolean(authLoading)} className="w-full flex items-center justify-center gap-2 bg-surface-container-low text-on-surface-variant rounded-xl px-6 py-3.5 font-bold text-sm hover:bg-surface-container-highest hover:text-primary transition-colors disabled:opacity-60">
              {authLoading === 'google-another' ? (
                <><span className="material-symbols-outlined text-[18px] animate-spin">progress_activity</span>Redirecting…</>
              ) : (
                <><span className="material-symbols-outlined text-[18px]">manage_accounts</span>Continue with Another Google Account</>
              )}
            </button>
            ...
            <button onClick={continueAsDemo} disabled={Boolean(authLoading)} className="w-full flex items-center justify-center gap-2 bg-surface-container-high text-on-surface-variant rounded-xl px-6 py-4 font-bold text-sm hover:bg-surface-container-highest hover:text-primary transition-colors disabled:opacity-60">
              {authLoading === 'demo' ? (
                <><span className="material-symbols-outlined text-[18px] animate-spin">progress_activity</span>Signing in…</>
              ) : (
                <><span className="material-symbols-outlined text-[18px]">science</span>Continue in Demo Mode</>
              )}
            </button>
          </div>
