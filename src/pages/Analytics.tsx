...
function buildChart(brews: Brew[]): ChartData {
  ...
  const W = 480, H = 190
  const mapX = (t: number) => (((t - minT) / rangeT) * (W - 40)) + 28
  ...
  const ePts: [number, number][] = sorted.filter(b => b.extraction).map(b => [mapX(new Date(b.date).getTime()), mapY(b.extraction!, 15, 25)])
...
export default function Analytics() {
  ...
  const bestBrews = useMemo(() => {
    return [...brews].sort((a, b) => b.rating - a.rating || new Date(b.date).getTime() - new Date(a.date).getTime()).slice(0, 2)
  }, [brews])
                ...
                {bestBrews.map((b, i) => (
                  <div key={b.id} className="group bg-surface-container-lowest rounded-xl overflow-hidden shadow-[0_4px_20px_rgba(62,39,35,0.04)] hover:shadow-xl transition-all">
                    ...
                    <div className="p-6">
                      ...
                      <div className="flex gap-0.5 mb-3"><Stars rating={b.rating} size="text-xs" /></div>
